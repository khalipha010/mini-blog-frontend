...
function PostList() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const sortedPosts = postsWithStatus.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
      setPosts(sortedPosts);
      setLikedPosts(
        postsWithStatus.reduce((acc, post) => ({ ...acc, [post.id]: post.isLiked }), {})
      );
      setMutedPosts(
        postsWithStatus.reduce((acc, post) => ({ ...acc, [post.id]: post.isMuted }), {})
      );
      ...
      await Promise.all(
        postsWithStatus.map(async (post) => {
          try {
            ...
            commentsData[post.id] = commentResponse.data.map((comment) => ({
              ...comment,
              pinned: comment.pinned || false,
            }));
            initialSort[post.id] = 'newest';
            initialVisible[post.id] = 5;
            const collectLikesAndCollapse = (comments) => {
              comments.forEach((comment) => {
                commentLikesData[comment.id] = comment.isLiked || false;
                if (comment.replies?.length > 0) {
                  initialCollapsed[`${post.id}-${comment.id}`] = false; // Expand by default
                }
  ...
  const handleCommentSubmit = async (postId, parentCommentId = null) => {
    ...
    try {
      ...
      setComments((prev) => ({
        ...prev,
        [postId]: commentResponse.data.map((comment) => ({
          ...comment,
          pinned: comment.pinned || false,
        })),
      }));
      ...
      const collectLikesAndCollapse = (comments) => {
        comments.forEach((comment) => {
          ...
          if (comment.replies?.length > 0) {
            initialCollapsed[`${postId}-${comment.id}`] = false; // Expand by default
          }
          if (comment.replies) collectLikesAndCollapse(comment.replies);
        });
      };
      ...
      setVisibleComments((prev) => ({ ...prev, [postId]: prev[postId] || 5 }));
      addToast(parentCommentId ? 'Reply added!' : 'Comment added!', 'success');
    } catch (error) {
  ...
  const handlePinComment = async (postId, commentId) => {
    ...
    try {
      ...
      if (!comment) throw new Error('Comment not found locally');

      console.log(`Attempting to ${comment.pinned ? 'unpin' : 'pin'} comment ${commentId} for post ${postId}`);
      ...
      console.log(`Successfully updated comments for post ${postId}`);
    } catch (error) {
      console.error('Pin comment error:', error);
      if (error.response?.status === 403) {
        addToast('Only the post author can pin/unpin comments.', 'error');
      } else if (error.response?.status === 404) {
        addToast('Comment not found. It may have been deleted.', 'error');
      } else if (error.response?.status === 400) {
        addToast('Invalid comment ID.', 'error');
      } else {
        addToast(error.response?.data.message || 'Failed to pin/unpin comment', 'error');
      }
    }
  };
  ...
  const handleLoadMoreComments = (postId) => {
    setVisibleComments((prev) => ({ ...prev, [postId]: (prev[postId] || 5) + 5 }));
  };

  const renderComment = (comment, postId, depth = 0) => {
    const hasReplies = comment.replies?.length > 0;
    const isCollapsed = collapsedComments[`${postId}-${comment.id}`] || false;
    ...
    return (
      <motion.div
        ...
        className={`pt-3 ${depth > 0 ? 'ml-6 pl-4 border-l-2 border-[var(--border)]' : ''} ${
          comment.pinned ? 'rounded-lg p-2 shadow-sm border border-[var(--accent)]' : ''
        }`}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {comment.pinned && (
                <span className="text-[var(--accent)] font-medium flex items-center gap-1 text-sm bg-[var(--primary-bg)] px-2 py-1 rounded-full">
            ...
            <div className="mt-2 flex items-center gap-4 text-sm flex-wrap">
              ...
              {user?.id === posts.find((p) => p.id === postId)?.user_id &&
                !deletedCommentIds.includes(comment.id) && (
                  <motion.button
                    onClick={() => handlePinComment(postId, comment.id)}
                    className="text-[var(--accent)] hover:underline flex items-center gap-1"
                    whileHover={{ x: 2 }}
                    whileTap={{ scale: 0.95 }}
                  >
              ...
              {hasReplies && (
                <motion.button
                  ...
                  className="text-[var(--accent)] font-medium flex items-center gap-1 px-3 py-1.5 rounded-md bg-[var(--secondary-bg)] hover:bg-[var(--primary-bg)] shadow-sm"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                        ...
                        {showDropdown[post.id] && (
                          ...
                          >
                            ...
                            (post.shared_from_post_id &&
                              user.id === post.originalPoster?.user_id) ? (
                              <>
                                <motion.button
                                  onClick={() => {
                    ...
                    <div className="p-4 space-y-4">
                      {comments[post.id]?.length > 0 ? (
                        <>
                          ...
                          <div
                            ref={(el) => (commentSectionRefs.current[post.id] = el)}
                            className="space-y-4 overflow-x-auto scrollbar-thin scrollbar-thumb-[var(--accent)] scrollbar-track-[var(--secondary-bg)] md:overflow-x-hidden"
                          >
